...
router.get('/', (req, res) => {
  restaurantModel.all('SELECT *, NULL as menu_urls FROM restaurant', (err, rows) => {
    if (err) {
...
router.post('/', (req, res) => {
  const {photo_url, menu_urls, name, description, price_range, cuisine, address, phone_number, seat_capacity } = req.body;

  // Validate required fields
  if (!photo_url || !menu_urls || !name || !description || !price_range || !cuisine || !address || !phone_number || !seat_capacity) {
  ...
  restaurantModel.run(
    ...
    [photo_url, menu_urls, name, description, price_range, cuisine, address, phone_number, seat_capacity],
    function (err) {
      if (err) {
        res.status(500).json({ error: err.message });
        return;
      }
      res.json({ id: this.lastID, photo_url, menu_urls, name, description, price_range, cuisine, address, phone_number, seat_capacity });
    }
  );
});
