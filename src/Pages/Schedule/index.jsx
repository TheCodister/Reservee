...
const TimeTable = ({reserveList}) => {
  ...
  const tableData = Array.from({ length: itemsPerPage}, () => new Array(itemsPerPage).fill(""));
  console.log(tableData)
  // Fill the table with data from reserveList
  reserveList.forEach(({ name, numOfPeople, tableNumber, timeSlot }) => {
    console.log(startIdx, " ", endIdx)
    if (timeSlot >= startIdx && timeSlot < endIdx) {
      tableData[tableNumber - 1][timeSlot - startIdx - 1] = (
        <div className="reserveChosenSlot">
          {name}
          <br />
          People: {numOfPeople}
        </div>
          ...
          {tableData.map((row, rowIndex) => (
            <tr key={rowIndex}>
              {row.map((data, colIndex) => (
                <td className="reserveData" key={colIndex}>{data}</td>
              ))}
            </tr>
          ))}
        </tbody>
...
const CustomerReview = ({reviewObject}) => {
  // reviewObject: reviewRating, reviewDate, reviewName, reviewDetail
...
const Schedule = () => {
  ...
  const reserveList = [
    {name: "Alice", numOfPeople: 2, tableNumber: 1, timeSlot: 1},
    {name: "David", numOfPeople: 3, tableNumber: 2, timeSlot: 1},
    {name: "Bella", numOfPeople: 4, tableNumber: 1, timeSlot: 2},
    {name: "Anna", numOfPeople: 2, tableNumber: 1, timeSlot: 3},
    {name: "Marry", numOfPeople: 3, tableNumber: 2, timeSlot: 3},
    {name: "Clinton", numOfPeople: 3, tableNumber: 3, timeSlot: 3},
    {name: "Clinton", numOfPeople: 3, tableNumber: 4, timeSlot: 3},
    {name: "Clinton", numOfPeople: 3, tableNumber: 4, timeSlot: 10},
  ]
